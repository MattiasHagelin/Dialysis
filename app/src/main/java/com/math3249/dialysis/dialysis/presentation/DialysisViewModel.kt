...
class DialysisViewModel(
    ...
    private fun getEntries(){
        viewModelScope.launch {
            repository.getDialysisEntries(session.groupId).collect {
                when {
                    it.isSuccess -> {
    ...
    fun updateDialysisEntry(data: DialysisEntry) {
        viewModelScope.launch {
            repository.updateDialysisEntry(data, session.groupId)
        }
    }

    fun deleteDialysisEntry(key: String) {
        viewModelScope.launch {
            repository.deleteDialysisEntry(key, session.groupId)
        }
    }
    ...
    fun addEntry() {
        viewModelScope.launch {
            ...
            repository.addEntry(entry, session.groupId)
        }
    }
